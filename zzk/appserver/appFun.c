...
int app_init()
{
	...
	if (val == 1)
	{
		printf("led mib is on\n");
		system("echo 1 > /proc/gpio");
	}
	else
	{
		printf("led mib is off\n");
...
int get_mesh_json_sta_info(P_MESH_DEV_INFO mesh_json)
{
	...
	{  
		...
		for(i=0; i<size; i++)  
		{
			...
			{
				//printf("master is %s\n", device_name);
				strcpy(mesh_json->sta_info[i].master, device_name);
			}
			mesh_json->sta_num++;
	...
	{ 
		int size = cJSON_GetArraySize(arrayItem);  
		printf("[%s][%d]child mesh has [%d] childs.\n",__FUNCTION__, __LINE__, size); 
...
int send_child_json(int fd, P_MESH_DEV_INFO mesh_json, char *pchildx_mac)
{
	...
	else
	{
		...
		memset(buffer, 0, sizeof(buffer));
		printf("[%s][%d]neighbor mac %s, rssi %s\n", 
