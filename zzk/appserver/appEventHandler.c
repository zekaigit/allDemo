...
int get_wl_environment_tbl(APP_ENV_INFO_T *p_env_sta_info)
{
	...
	printf("enter wlsiteservertbl 5G\n");
	...
	for (i=0; i<pStatus->number && pStatus->number!=0xff; i++) 
	{
		...
		p_env_sta_info->sta_info[i].bdBrates = pBss->bdSupportRates;
...
int get_app_request_type(const char* sourceStr)
{
	...
	printf("[appserver][%s-%d]source targetStr = %s strlen(targetStr) = %d\n",
...
{	
	...
	for (; i < iCount; ++i) 
	{
		...
		if (!strcmp("on", reboot))
		{
			send(fd, "errorCode=1", strlen("errorCode=1"), 0) ;
			sleep(1);
			system("reboot");
		}
...
{	
	...
	for (; i < iCount; ++i) 
	{
		...
		if (!strcmp(upgrade, "on"))
		{
			...
			printf("main dev upgrade url = %s\n", url);
			sprintf(buffer, "wget -O %s %s", "/tmp/fw.bin", url);
			system(buffer);
		}
		...
		{  
			...
			if (!strcmp(upgrade, "on"))
			{
				...
				char *url = tmp->valuestring;
				printf("child dev upgrade url = %s\n", url);
			}
...
{	
	...
	for (; i < iCount; ++i) 
	{
				...
				*((char *)&urlEntrytmp) = (char)j;
				if ( apmib_get(MIB_URLFILTER_TBL, (void *)&urlEntrytmp))
				{
					...
					if (strlen(urlEntry.urlAddr) == strlen(urlEntrytmp.urlAddr))
					{
						...
						{
							printf("the url is on table ,not set again !!!\n");
							send_return_code(fd, APP_SETTING_SUC);
							return EXIT_SUCCESS;
						}
					}
		...
		{
			printf("MIB_IPFILTER_DEL pEntry.urlAddr = %s!!!\n", urlEntry.urlAddr);
			printf("MIB_IPFILTER_DEL pEntry.ruleMode = %d!!!\n", urlEntry.ruleMode);
			apmib_set(MIB_URLFILTER_DEL, (void*)&urlEntry);
...
{	
	...
	for (; i < iCount; ++i) 
	{
		...
		{
			printf("MIB_IPFILTER_DEL pEntry.comment = %s!!!\n", ipEntry.comment);
			ipEntry.protoType = PROTO_BOTH;
			int j = 0;
			...
			for(j=1; j<=entryNum; j++)
			{
				...
				*((char *)&ipentrytmp) = (char)j;
				if ( apmib_get(MIB_IPFILTER_TBL, (void *)&ipentrytmp))
				{
					if ((*((unsigned int*)ipentrytmp.ipAddr)) == (*((unsigned int*)ipEntry.ipAddr)))
					{
						printf("MIB_IPFILTER_DEL ipentrytmp.comment = %s!!!\n", ipentrytmp.comment);
		...
		if ( !strcmp(method, "add") )
		{
			...
			for(j=1;j<=entryNum;j++)
			{
				...
				*((char *)&macEntrytmp) = (char)j;
				...
				{
					printf("[%s-%d]entryNum = %d MAC0 %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	if (x < 0 || x > PARENT_CONCTRL_GROUPS_NUMBER)
{
	...
	printf("set   MIB_PARENT_CONCTRL_GROUP0_NAME+%d = %s\n", x, p_group->name);
	apmib_get(MIB_PARENT_CONCTRL_VALID_NUM, (void *)value);
	printf("get old valid  num = %s\n", value);
	sprintf(buffer, "%d", atoi(value)+1);
	printf("set valid  num = %s\n", buffer);
	...
	printf("set MIB_PARENT_CONCTRL_GROUP0_NUM+%d = %s\n", x, buffer);
	...
	for (i=0; i < mac_num; i++)
	{
		...
		if (strlen(p_group->mac_list[i])<=0)
			return -1;
		apmib_get(MIB_PARENT_CONCTRL_GROUP0_LIST+x, (void *)value);
		printf("===>get MIB_PARENT_CONCTRL_GROUP0_LIST+%d = %s\n", x, value);
...
int parent_conctrl_del_group(const char *group_name)
{
	...
	apmib_get(MIB_PARENT_CONCTRL_VALID_NUM, (void *)value);
	printf("get PARENT_CONCTRL_VALID_NUM = %s\n", value);
	sprintf(buffer, "%d", atoi(value)-1);
	apmib_set(MIB_PARENT_CONCTRL_VALID_NUM, (void *)buffer);
	printf("set PARENT_CONCTRL_VALID_NUM = %s\n", buffer);
...
{	
	...
	printf("enter set_add_parental_conf iCount = %d\n", iCount);
	...
	{  
		...
		for(i=0; i<dev_list.dev_num; i++)
		{
			...
			{
				if (strlen(dev_list.hostname[i])!=0)
					sprintf(tmpInfo, "%s", dev_list.hostname[i]);
				else 
...
{	
	printf("======  set_del_all_parental_conf \n");
	char send_conf_buff[2048] = {0};
	int iCount = cJSON_GetArraySize(jason_obj);  
	int i = 0;	
	int j = 0;
	...
	printf("enter set_del_all_parental_conf iCount = %d\n", iCount);
...
{	
	...
	printf("enter set_modify_parental_conf iCount = %d\n", iCount);
	...
	{  
		...
		*((char *)pEntry) = (char)select_id;
		...
		for(i=0; i<dev_list1.dev_num; i++)
		{
			...
			{
				if (strlen(dev_list1.hostname[i])!=0)
					sprintf(tmpInfo, "%s", dev_list1.hostname[i]);
				else 
...
{	
	...
	{  
		...
		printf("ssid = %s,pskValue = %s,broadcast = %s\n",ssid,pskValue,broadcast);
		...
		printf("ssid_2g=%s,pwd_2g=%s,ssid_5g=%s,pwd_5g=%s\n",ssid_2g,pwd_2g,ssid_5g,pwd_5g);
...
#if 0
{
	...
	printf("MIB_DEV_RESTART_ENABLE =%s\n", buffer);
	if(strlen(buffer) <= 0)
	{
...
int send_upgrade_conf_to_client(int fd)
{
	...
	cJSON_AddStringToObject(parameters, "mainDevName", "test");
	...
	for(i = 0; i < childNum; i++)
	{
		...
		cJSON_AddItemToObject(parameters, childx, array = cJSON_CreateArray());
		cJSON_AddItemToArray(array, arrayobj = cJSON_CreateObject());
		cJSON_AddStringToObject(arrayobj, "childName", "test");
...
#if 0
{
	...
	if(groupNum < 0)
	{
		printf("valid groupNum error is %d\n", groupNum);
		goto retError;
	}
	else if(groupNum == 0)
	{
		...
		for (z=0; z < PARENT_CONCTRL_GROUPS_NUMBER; z++)
		{
			...
			else
			{
				for(j=0; j < groupx_num; j++)
				{
					...
					//printf("group_info.mac_list[%d] = %s\n", j, group_info.mac_list[j]);
				}
			}
...
int send_wlan_basic_conf_to_client(int fd)
{
	...
	if(wlanif >= 0)
	{
		...
		{
			printf("apmib_get MIB_WLAN_HIDDEN_SSID error!\n");
			goto retError;
		}
		if (val==1)
...
int parse_Set_conf( int fd,char *jasonData)
{
	...
	{
		printf("====== parse network connection type  \n");
